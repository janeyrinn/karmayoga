...
def cache_checkout_data(request):
    """A view to retrieve the cart from the session"""
    try:
...
def checkout(request):
    """
    A view to
    render templates, form and process via stripe
    """
...
# attaches profile to order
    if request.user.is_authenticated:
        profile = UserProfile.objects.get(user=request.user)
        order.user_profile = profile
        order.save()

# Save the users info if box is checked
        if save_info:
            profile_data = {
